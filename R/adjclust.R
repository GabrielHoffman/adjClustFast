#' @useDynLib adjClustFast, .registration = TRUE
NULL
...
#' fit1 <- adjClustFast(sim, "similarity")
#' plot(fit1)
#'
#' ## similarity, h < p-1
#' fit2 <- adjClustFast(sim, "similarity", h = 2)
#' plot(fit2)
#'
#' ## dissimilarity
...
#' fit3 <- adjClustFast(dist, "dissimilarity")
#' plot(fit3)
#'
#' ## dissimilarity, h < p-1
...
#' @import adjclust
adjClustFast <- function(mat, type = c("similarity", "dissimilarity"), 
                     h = ncol(mat) - 1) {
  UseMethod("adjClustFast")
}

#' @export
adjClustFast.matrix <- function(mat, type = c("similarity", "dissimilarity"), 
                            h = ncol(mat) - 1) {
  if (!is.numeric(mat))
...
#' @export
adjClustFast.dsyMatrix <- function(mat, type = c("similarity", "dissimilarity"), 
                            h = ncol(mat) - 1) {
  if (!(isSymmetric(mat)))
    stop("Input matrix is not symmetric")
  res <- run.adjclust(mat, type = type, h = h)
...
#' @export
adjClustFast.dgeMatrix <- function(mat, type = c("similarity", "dissimilarity"), 
                               h = ncol(mat) - 1) {
  type <- match.arg(type)
  if (!(isSymmetric(mat))) {
    stop("Input matrix is not symmetric")
...
#' @export
adjClustFast.dsCMatrix <- function(mat, type = c("similarity", "dissimilarity"), 
                               h = ncol(mat) - 1) {
  type <- match.arg(type)
...
#' @export
adjClustFast.dgCMatrix <- function(mat, type = c("similarity", "dissimilarity"), 
                               h = ncol(mat) - 1) {
  if (!(isSymmetric(mat))) {
    stop("Input matrix is not symmetric")
  } else {
...
#' @export
adjClustFast.dsTMatrix <- function(mat, type = c("similarity", "dissimilarity"), 
                               h = ncol(mat) - 1) {
  type <- match.arg(type)
  if (type == "dissimilarity")
    stop("'type' can only be 'similarity' with sparse Matrix inputs")
...
#' @export
adjClustFast.dgTMatrix <- function(mat, type = c("similarity", "dissimilarity"), 
                               h = ncol(mat) - 1) {
  type <- match.arg(type)
  if (!(isSymmetric(mat))) {
    stop("Input matrix is not symmetric")
...
#' @export
adjClustFast.dist <- function(mat, type = c("similarity", "dissimilarity"), 
                          h = ncol(mat) - 1) {
  type <- match.arg(type)
  if (type != "dissimilarity")
    message("Note: input class is 'dist' so 'type' is supposed to be 'dissimilarity'")
  mat <- as.matrix(mat)
  res <- adjClustFast.matrix(mat, type = "dissimilarity", h = h)
  return(res)
}

#' @import Matrix RcppArmadillo Rcpp
...
run.adjclust <- function(mat, type = c("similarity", "dissimilarity"), h, strictCheck=FALSE) {
  ...
  if( strictCheck ){
    checkCondition = eval(parse(text="adjclust:::checkCondition")) 
  ...
  buildHeap = eval(parse(text="adjclust:::buildHeap")) 
